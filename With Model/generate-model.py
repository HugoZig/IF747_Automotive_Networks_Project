...
def generateModel(path):   
    ...
    df.head(20)
    #model = LocalOutlierFactor(novelty=True, n_neighbors=500, contamination=0.00000000001)
    #model = OneClassSVM()
    model = IsolationForest(n_estimators=200,  max_samples='auto', bootstrap=True,  contamination=0.1, max_features=2)
    model.offset_ = -0.5
    model.fit(df.values)
