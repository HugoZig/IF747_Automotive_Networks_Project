    ...
    for msg in bus:
        ...
        if data:
            if not data[2]:
                ...
                predictResponse = model.predict(np.array([[msg.arbitration_id,int.from_bytes(msg.data, byteorder='little')]]).reshape(1, -1) )
                if predictResponse[0] == -1: # anomalia detectada
                    print(f"({msg.timestamp}) {msg.channel} {msg.arbitration_id}#{msg.data} E2 => anomalia detectada")
