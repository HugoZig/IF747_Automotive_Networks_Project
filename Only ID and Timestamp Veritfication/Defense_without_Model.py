...
logging.basicConfig(filename=f"bus.log", level=logging.INFO, format='%(message)s')

def createLogLine(msg, type = ""):
    payload = str(binascii.hexlify(msg.data[::-1])).replace("b\'", "").replace("\'", "")
    if type != "":
...
with can.Bus(channel="can0",   interface="socketcan",  bitrate=50000) as bus:
    for msg in bus:
        # print([msg.arbitration_id, int.from_bytes(msg.data, byteorder='little') ], " ",[ hex(msg.arbitration_id), msg.data] ) 
        ...
        if data:
            if not data[2]:
                ...
                print(f"({msg.timestamp}) {msg.channel} {msg.arbitration_id}#{msg.data} E0 => timestamp malicioso")
                createLogLine(msg, 'E0')
                bus.send(msgToSend)
            data[3] = msg.timestamp
            ids.update({arbitration_id:data})
        else: 
            print(f"({msg.timestamp}) {msg.channel} {msg.arbitration_id}#{msg.data} E1 => ID inexistente") 
